...
pub fn check(
    ...
    db.send(m::Get {sw_token: sw_token,user_id: auth_user.id}).from_err()
    .and_then(move |res| {
        //let getWithKey = res.unwrap();
        //let  chk_type = getWithKey.get_type();

        let  chk_type = match &res {
             Ok(getWithKey) => {
                (getWithKey.get_type(),getWithKey.shop_id.clone())
            }
            Err(e) => { 
                println!("err================err==============================");
        ...
        //Ok(HttpResponse::Ok().json(getWithKey))
    })
