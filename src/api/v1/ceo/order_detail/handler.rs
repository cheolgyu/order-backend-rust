...
use crate::schema::order::dsl::{order as tb_order, state as order_state};
use crate::schema::order_detail::dsl::{order_detail as tb};
use crate::schema::shop::dsl::shop as tb_shop;
use actix::Handler;
...
impl Handler<model::New> for DbExecutor {
    ...
    fn handle(&mut self, msg: model::New, _: &mut Self::Context) -> Self::Result {
        let conn = &self.0.get()?;
        let od = tb_order.find(&msg.order_id).load::<Order>(conn)?.pop();
        let s = tb_shop.find(&msg.shop_id).load::<Shop>(conn)?.pop();
        match s {
            Some(s_ok) => {
                match od {
                    Some(od_ok) => {
