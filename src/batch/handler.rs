...
impl Handler<ComeFind> for DbExecutor {
    ...
    fn handle(&mut self, _msg: ComeFind, _: &mut Self::Context) -> Self::Result {
        let conn = &self.0.get()?;

        let list =
            sql_query("select * from view_comfind_info ").get_results::<ComeFindRes>(conn)?;
...
impl Handler<AutoCancel> for DbExecutor {
    ...
    fn handle(&mut self, _msg: AutoCancel, _: &mut Self::Context) -> Self::Result {
        let conn = &self.0.get()?;
